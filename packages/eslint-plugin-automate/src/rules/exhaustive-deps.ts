...
const RULE_NAME = 'exhaustive-deps'

type Options = [Record<string, string>]

type MessageIds =
	| 'testMessage'
	| 'only_global_func_dep'
	...
	meta: {
		...
		schema: [
			{
				...
				useEffectRemoveUnnecessaryDependencies: false,
				useEffectDisregardFunctions: false,
				properties: {
					additionalHooks: {
						type: 'string',
					},
					hooksToIncludeAllDependencies: {
						type: 'string',
					},
					additionalStableHandlers: {
						type: 'string',
					},
					enableDangerousAutofixThisMayCauseInfiniteLoops: {
						type: 'boolean',
					},
					useEffectRemoveUnnecessaryDependencies: {
						type: 'boolean',
					},
					useEffectDisregardFunctions: {
						type: 'boolean',
					},
				},
			},
		...
		messages: {
			...
			'will be noted as local dependency causing a selector to not be cached globally':
				'will be noted as local dependency causing a selector to not be cached globally',
			only_global_func_dep:
				'component functions cannot be used in selectors - only if they are declared as global by appending __global to then name, so that they are not dependend on local vars and so the selector can be cached',
		},
	},
	create: (context) => {
		...
		// CHANGE V3
		const hooksToIncludeAllDependencies =
			context.options && context.options[0] && context.options[0].hooksToIncludeAllDependencies
				? new RegExp(context.options[0].hooksToIncludeAllDependencies)
				: undefined

		// CHANGE V1

		const additionalStableHandlers =
			context.options && context.options[0] && context.options[0].additionalStableHandlers
				? new RegExp(context.options[0].additionalStableHandlers)
				: undefined
		...
		const useEffectRemoveUnnecessaryDependencies =
			(context.options && context.options[0] && context.options[0].useEffectRemoveUnnecessaryDependencies) ||
			false

		const useEffectDisregardFunctions =
			(context.options && context.options[0] && context.options[0].useEffectDisregardFunctions) || false
		...
		function visitFunctionWithDependencies(
			...
			reactiveHookName,
			isEffect,
			isHookIncludeAllDeps
		) {
			// CHANGE V2 enable async effect
			if (isEffect && node.async && !reactiveHookName.startsWith('useRecoilEffect')) {
				reportProblem({
					node: node,
					message:
			...
			function isStableKnownHookValue(resolved) {
				...
				const { name } = callee
				if ((name === 'useRef' || options.additionalStableHandlers?.test(name)) && id.type === 'Identifier') {
				...
				if (setStateInsideEffectWithoutDeps) {
					...
					reportProblem({
						...
						message:
							...
							`To fix this, pass [${suggestedDependencies.join(
								', '
			...
			const { suggestedDependencies, unnecessaryDependencies, missingDependencies, duplicateDependencies } =
			...
			// CHANGE V2 we now do this for effects too.
			...
			function getWarningMessage(deps, singlePrefix, label, fixVerb) {
				...
				return `${(deps.size > 1 ? '' : `${singlePrefix} `) + label} ${
				...
				missingDependencies.forEach((missingDep) => {
					...
					for (let i = 0; i < usedDep.references.length; i++) {
						...
						if (
							id != null &&
							id.parent != null &&
							(id.parent.type === 'CallExpression' || id.parent.type === 'OptionalCallExpression') &&
							id.parent.callee === id
						) {
							isFunctionCall = true
							break
		...
		function visitCallExpression(node) {
			...
			const isHookIncludeAllDeps = RegExp(hooksToIncludeAllDependencies).test(reactiveHookName)
...
function collectRecommendations({
	...
	// CHANGE V1
	let [dependencies, functionDeps] = options.useEffectDisregardFunctions
		? [
				new Map(
					Array.from(_dependencies).filter(
						([key, value]) => !isNodeTypeIncludesFunction(context, value.references[0].identifier)
					)
				),
				new Map(
					Array.from(_dependencies).filter(([key, value]) =>
	...
	// 	fnfndep: functionDeps.get('fn')?.references?.[0],
	// 	fndep: dependencies.get('fn')?.references?.[0],
	// 	'objectFn.fn': dependencies.get('objectFn.fn')?.references?.[0],
	// })
	// console.log({ _dependencies, dependencies, functionDeps })
	...
	declaredDependencies.forEach(({ key }) => {
		...
		// CHANGE V1
		if (satisfyingDependencies.has(key) && (!stableDependencies.has(key) || isHookIncludeAllDeps)) {
			...
			// CHANGE V1 by default it we will go to else because of useEffectRemoveUnnecessaryDependencies
			if (
				!functionDeps.has(key) &&
				!options.useEffectRemoveUnnecessaryDependencies &&
				isEffect &&
	...
	if (isHookIncludeAllDeps) {
		for (let depKey of satisfyingDependencies) {
			if (!declaredDependencies.find(({ key }) => key === depKey)) {
				missingDependencies.add(depKey)
			}
		}
	}
	// console.log({ _dependencies, declaredDependencies, suggestedDependencies, satisfyingDependencies, unnecessaryDependencies })
 ...
 */
function getDependency(node, isHookIncludeAllDeps) {
	if (
		(node.parent.type === 'MemberExpression' || node.parent.type === 'OptionalMemberExpression') &&
		node.parent.object === node &&
		// CHANGE V3
		(node.parent.property.name !== 'current' || isHookIncludeAllDeps) &&
		...
		return getDependency(node.parent, isHookIncludeAllDeps)
	} else if (
...
export const exhaustiveDeps = { rule, RULE_NAME }
